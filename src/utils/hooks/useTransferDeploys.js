...
export const useDeploysWithStatus = ({ symbol, publicKey, status } = {}) => {
	...
	const transfersDeployList = useSelector((state) => getDeploysTransfer(state, {publicKey, symbol}));
	const pendingTransferDeployHash = transfersDeployList.filter((deploy) => deploy.status === 'pending').map((deploy) => deploy.deployHash);

	useEffect(() => {
		if (pendingTransferDeployHash && pendingTransferDeployHash.length) {
			(async () => {
				const data = await apis.getTransferDeploysStatusAPI({deployHash: pendingTransferDeployHash});
