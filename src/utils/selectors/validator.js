...
const addValidatorIcon = memoizeOne((validators) => {
    return validators.map((validator) => {
        return { ...validator, icon: getBase64IdentIcon(validator.public_key, { size: 100 }) };
    });
});
...
export const getListValidators = (searchTerm) => createSelector(stakingReducer, ({listValidators}) => {
    if (!listValidators) {
        return [];
    }
