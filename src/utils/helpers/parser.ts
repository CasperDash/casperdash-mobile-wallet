...
export function getDeployType(deploy: DeployUtil.Deploy) {
  ...
  if (deploy.isTransfer()) {
    return DeployTypes.Transfer;
  }

  if (deploy.session.isModuleBytes()) {
    return DeployTypes.Wasm;
  }

  if (deploy.session.isStoredContractByHash() || deploy.session.isStoredContractByName()) {
    return DeployTypes.ContractCall;
  }

  return DeployTypes.ContractPackageCall;
}
