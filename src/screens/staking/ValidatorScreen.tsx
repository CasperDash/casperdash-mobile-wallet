...
import ValidatorItem from 'screens/staking/ValidatorItem';
...
function ValidatorScreen() {
    ...
    // @ts-ignore
    const isLoading = useSelector((state: any) => checkIfLoadingSelector(state, [stakingTypes.GET_VALIDATORS_INFORMATION]));
    // @ts-ignore
    const isRefreshing = useSelector((state: any) => checkIfRefreshingSelector(state, [stakingTypes.GET_VALIDATORS_INFORMATION]));

    const [search, setSearch] = useState('');
    const listValidators = useSelector(getListValidators(search));

    const onReload = () => {
        dispatch(allActions.staking.getValidatorsInformation({refreshing: true}, (error: any, _: any) => {
            if (error) {
        ...
        <CLayout
            ...
            <Col mt={16} style={styles.container}>
                ...
                <FlatList
                    ...
                    contentContainerStyle={{paddingBottom: insets.bottom + scale(20), minHeight: scale(400)}}
                    data={listValidators}
